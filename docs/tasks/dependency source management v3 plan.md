...
**Completion notes:** In progress as of 2026-07-11. Added an optional v2-only `migration_hints` object with platform dependency IDs and logical dependency/component hints. Component hints identify legacy dependency rows by index and use the strict v3 enums for kind, role, semantic scopes, artifact treatment, and data-run policy. V1 normalization initializes hints as absent, and normal v2 engine conversion deliberately discards them. Added deterministic structured diagnostics carrying a field path, message, legacy row index/configuration/coordinate where applicable, candidate values, and remediation. Validation accumulates missing fields, duplicate IDs, duplicate/out-of-range row ownership, uncovered rows, empty/duplicate scopes, and invalid platform references in one pass. Acquisition evidence now requires consistent requested/resolved coordinates and cache paths across grouped rows, parses CurseMaven slug/project/file IDs, requires exactly one matching root artifact, and validates repository provenance. V3 source providers may remain empty rather than inventing unavailable source metadata; later source commands can add maintained providers. Parsed v2 documents carry the report for the future migration command. V1 migration presentation and successful strict-v3 construction remain before this item is complete. The full gate passes 166 tests.
...
**Completion notes:** In progress as of 2026-07-11. Added the typed `dependency migrate --branch <selector> --check` command with exact-one-worktree selection. It reads through the versioned schema dispatcher, normalizes v1 to v2 in memory for diagnostics, reports every structured v2 correction with candidates and row context, treats v3 as already current, and never writes in check mode. Complete v2 evidence now constructs a strict v3 candidate, serializes it through Facet, reparses and validates it, and exposes it as the deterministic check preview. Write mode uses a synced sibling temporary file, verifies the source lockfile has not changed concurrently, and atomically persists the validated candidate; no side backup is created because Git is authoritative, while every failure preserves the original. Platform components without legacy dependency rows use explicit v2-only acquisition and artifact-index hints. Verified the initial check through `cargo run -- dependency migrate --branch 1.19.2 --check`; populating the actual 1.19.2 hints and running the successful migration remain. Parser, round-trip, atomic replacement, and concurrent-change coverage pass; the full gate passes 169 tests.
