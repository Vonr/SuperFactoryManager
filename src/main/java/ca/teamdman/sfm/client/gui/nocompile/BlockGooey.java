package ca.teamdman.sfm.client.gui.nocompile;
...
@EventBusSubscriber(modid = SFM.MOD_ID, value = CLIENT)

public class BlockGooey<E extends TileEntity> {
	...
	private final @Nonnull
	Matrix4d         pitchRot      = new Matrix4d();
	private final @Nonnull
	Matrix4d         yawRot        = new Matrix4d();
	private @Nonnull
	NNList<BlockPos> configurables = new NNList<BlockPos>();
	...
	private @Nonnull
	NNList<BlockPos> neighbours = new NNList<BlockPos>();
...
//	}
	...
	protected @Nonnull
	String getLabelForMode(@Nonnull IoMode mode) {
	...
	private void renderScene() {
		...
		for (BlockRenderLayer pass : BlockRenderLayer.values()) {
...
//		ForgeHooksClient.setRenderPass(-1);
	...
	private void doTileEntityRenderPass(Stream<BlockPos> blocks, final BlockRenderLayer pass) {
		blocks.forEach(pos -> {
			...
			if (tile != null) {
				...
				//						if (tile.getClass() == ChestTileEntity.class) {
	...
	private void doWorldRenderPass(@Nonnull Vector3d trans, Stream<BlockPos> blocks, final @Nonnull BlockRenderLayer layer) {
	...
	public void renderBlock(@Nonnull BlockState state, @Nonnull BlockPos pos, @Nonnull IEnviromentBlockReader blockAccess, @Nonnull BufferBuilder worldRendererIn) {

		try {
			BlockRendererDispatcher blockrendererdispatcher = mc.getBlockRendererDispatcher();
			BlockRenderType         type                    = state.getRenderType();
			if (type != BlockRenderType.MODEL.MODEL) {
	...
	public static class SelectedFace<E extends TileEntity> {
		...
		public SelectedFace(@Nonnull E config, @Nonnull Direction face) {
			this.config = config;
			this.face = face;
		}
