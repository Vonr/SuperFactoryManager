...
class MatcherDrawerItem<T extends FlowComponent & FlowDataHolder<? extends MovementMatcher>> extends
	FlowContainer {

	private final T DELEGATE;
	private final DisplayIcon ICON;
	private final MatchersSection PARENT;

	public MatcherDrawerItem(
		MatchersSection parent,
		T delegate
	) {
		...
		public void onSelectionChanged() {
			if (!Client.allowMultipleRuleWindows && isSelected()) {
				...
				PARENT.DRAWER.getChildren().stream()
					.filter(c -> c instanceof MatcherDrawerItem
						&& c != MatcherDrawerItem.this)
