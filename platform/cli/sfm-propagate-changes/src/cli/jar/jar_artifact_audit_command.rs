...
pub struct JarArtifactAuditCommand {
...
/// Run `jar audit-artifacts`.
///
/// # Errors
///
/// Returns an error if locked artifact cache/source provenance fails verification.
pub(super) fn invoke(command: JarArtifactAuditCommand) -> eyre::Result<()> { // todo(2026-06-16) should be instance method?
    ArtifactAuditCommand::new(command.into_options()?).invoke()
}
