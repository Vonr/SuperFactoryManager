...
fn write_lockfile_atomically(path: &Path, expected_input: &str, output: &[u8]) -> eyre::Result<()> {
    let parent = path
        .parent()
    ...
    if current != expected_input {
        eyre::bail!(
            "{} changed while migration was running; no files were modified",
            path.display()
        );
    }
    temporary
...
mod tests {
    ...
    fn atomic_write_preserves_concurrent_change() {
        ...
        assert!(error.to_string().contains("changed while migration"));
        assert_eq!(
